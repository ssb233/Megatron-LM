...
def _paired_intervals(
    ...
    configured_delay_ns = int(
        max(
            (
                float(event.get("bandwidth_seconds", 0.0))
                for event in events
                if event.get("event") == "delay_config"
            ),
            default=0.0,
        )
        * 1e9
        ...
        intervals.append(
            {
                ...
                "start_ns": start_ns,
                "end_ns": max(
                    int(completion["timestamp_ns"]),
                    start_ns + configured_delay_ns,
                ),
...
def _panel_timeline(
    ...
    axis.set_title(
        f"a  PP=4 trace timeline, 1.0 x F (iteration {representative_iteration})",
        loc="left",
        fontweight="bold",
    )
    ...
    axis.legend(
        handles=[
            ...
            Patch(
                color=COLORS["forward_comm"],
                label="Forward P2P + injected delay",
            ),
            Patch(
                color=COLORS["backward_comm"],
                label="Backward P2P + injected delay",
            ),
        ],
        ncol=4,
        loc="upper center",
...
def _panel_dependency(axis, summary: dict[str, Any], iteration: int) -> None:
    records = [
        ...
        and int(record["dependency_id"]) == 6
    ]
    if not records:
        axis.text(
            0.5,
    ...
    axis.set_xlabel("Time from first trigger completion (us)")
    axis.set_title(
        "b  Dependency 6: Comm_F_4_1_2 complete -> Comm_B_0_3_2 submit",
        loc="left",
        fontweight="bold",
    )
...
def _panel_duration(axis, rows: list[dict[str, Any]]) -> None:
    factors = [0.0, 0.5, 1.0]
    labels = ["0 x F", "0.5 x F", "1.0 x F"]
    values = [
        [
            float(row["bandwidth_seconds"]) * 1e6
            for row in rows
    ...
    axis.set_ylabel("Configured transfer delay (us)")
    axis.set_title(
        "c  Visualization-only transfer-delay configuration",
        loc="left",
        fontweight="bold",
    )
    axis.text(
...
def _write_figure_source_data(
    ...
    with output_path.open("w", encoding="utf-8", newline="") as stream:
        ...
        for row in rows:
            for metric, value in (
                ("communication_duration", row["communication_duration_us"]),
                (
                    "configured_transfer_delay",
                    float(row["bandwidth_seconds"]) * 1e6,
                ),
            ):
                writer.writerow(
...
def create_figure(
    ...
    plt.close(figure)
    svg_lines = outputs[0].read_text(encoding="utf-8").splitlines()
    ...
    caption.write_text(
        ...
        "1.0 x F artificial NCCL-stream transfer delay. Communication bars "
        "include the configured visualization window. Panel b shows the Gloo "
        "control path from Comm_F_4_1_2 completion to Comm_B_0_3_2 "
        "submission for dependency 6. Panel c reports the selected "
        "configured transfer delay at 0 x F, 0.5 x F, and 1.0 x F. Artificial "
        "transfer delay is for visualization only and is excluded from "
